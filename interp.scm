...
;;;;;;;;;;;;;;;; the interpreter ;;;;;;;;;;;;;;;;
  ...
  (define value-of
      ...
      (cases expression exp
        ...
        (queue-push-exp (exp1 exp2)
                  (let ((arr (expval->arr(value-of exp1 env)))
                        (val (value-of exp2 env)))                    
                      (newref val)
                      (setref! arr (+ (deref arr) 1))
                      ))

        (queue-pop-exp (exp1)
                      (let((arr (expval->arr(value-of exp1 env))))
                        (if (> (deref arr) 0)
                          (let ((deleted (read-array arr 0)))
                           (setref! (+ arr 1) (+ (deref (+ 1 arr)) 1))
                           (setref! arr (- (deref arr) 1))
                             deleted
                           )
                          (num-val -1))))
        ...
        (empty-queue?-exp (exp1)
                          (let((arr (expval->arr(value-of exp1 env))))
                            (if (= (deref arr) 0) (bool-val #t) (bool-val #f))))

        (print-queue-exp (exp1)
                          (let((arr (expval->arr(value-of exp1 env))))
                            (print-helper arr 0  (deref arr))))
        ...
        (stack-pop-exp (exp1)
                      (let((arr (expval->arr(value-of exp1 env))))
                        (if (> (deref arr) 0)
                          (let((deleted (read-array arr (- (deref arr) 1))))
                           (setref! (+ (- (deref arr) 1) (deref (+ 1 arr))) -1)                          
                           (setref! arr (- (deref arr) 1))
                           (optimize-store)
                            deleted
                           )
    ...
    (define print-helper (lambda (arr index length)
                 (if (= length 0) '()          
                  (if (= index (- length 1))
                      (display (expval->num(read-array arr index)))
                      (begin
                        (display (expval->num(read-array arr index)))
                        (display " ")
                        (print-helper arr (+ 1 index) length))
                      ))))
